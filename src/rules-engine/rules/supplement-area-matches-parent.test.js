...
const SUPPLEMENT_AREA_RULE_CONFIG = {
  baseActions: ['CLIG3', 'LIG1', 'LIG2', 'GRH6']
}
...
describe('supplementAreaMatchesParent', function () {
  ...
  test('should return false if the user doesnt have the parent action', function () {
    ...
    expect(result).toStrictEqual({
      passed: false,
      message:
        'Action code GRH7 requires an existing agreement with any of: CLIG3, LIG1, LIG2, GRH6'
    })
  })
  ...
  test('should return false if the user the areas dont match', function () {
    ...
    expect(result).toStrictEqual({
      passed: false,
      message:
        'Application is for GRH7 with an area of 100ha, the base action(s) LIG2 (101ha) is/are present. These areas should match.'
    })
  })
})
